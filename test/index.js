...
it('should return DOM element when using function', function () {
	...
	assert.ok(node1.matches(selector1), `Element doesn’t match selector ${selector1}`);
	assert.equal(node1.textContent.trim(), content1, `Element doesn’t have content "${content1}"`);

	const selector2 = 'td#rocky[louie="coco"]';
	const content2 = 'sassy';
	assert.ok(node2.matches(selector2), `Element doesn’t match selector ${selector2}`);
	assert.equal(node2.textContent.trim(), content2, `Element doesn’t have content "${content2}"`);

	const selector3 = 'option[value="josie"]';
	const content3 = 'penny';
	assert.ok(node3.matches(selector3), `Element doesn’t match selector ${selector3}`);
	assert.equal(node3.textContent.trim(), content3, `Element doesn’t have content "${content3}"`);

	const selector4 = 'legend#henry';
	const content4 = '<span><b>cash</b></span>';
	assert.ok(node4.matches(selector4), `Element doesn’t match selector ${selector4}`);
	assert.equal(node4.innerHTML.trim(), content4, `Element doesn’t have HTML "${content4}"`);

	const content5 = 'sydney';
	assert.ok(node5.nodeType === Node.TEXT_NODE, 'Element is not text node');
	assert.equal(node5.textContent.trim(), content5, `Element doesn’t have content "${content5}"`);
